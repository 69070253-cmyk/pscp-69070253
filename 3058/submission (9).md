...
```text
input :  รับตัวแปรจำนวนเต็ม 3 ตัว
output : แสดงจำนวนก้อนเล็กที่ใช้น้อยที่สุดและถ้าใช้ไม่พอให้ตอบ -1
constraints : ถ้ามีจำนวนไม่พอ ให้ตอบ -1
```
...
```text
step 1 : สร้างเป็นสมการขึ้นมาก่อน
step 2 : หลังจากนั้นกำหนดทีละตัว ถ้า goal > long + a จะให้ print -1 
step 3 : ต่อมากำหนดให้ goal > b จะให้ goal - long
step 4 : goal > (5 * (goal // 5)) + a ให้ goal//5 คืออิฐก้อนใหญ่ที่ควรใช้ 5 * (goal // 5) คือความยาวที่ได้จากอิฐก้อนใหญ่ ถ้านำ ความยาวที่ได้จากอิฐใหญ่ไป + a แล้วไม่ถึง แสดงว่าไม่พอให้ตอบ -1
step 5 : ถ้า goal < long จะได้ goal-(5*(goal//5)) แนวทางการคิดแบบเดียวกับ step 4 ต่างกันตรงที่อันนี้เป็น output
step 6 : goal == long ถ้าทั้งคู่เท่ากัน เท่ากับว่าไม่ต้องใช้ก้อนเล็กสักก้อน
```
...
```text
แก้โค้ดใหม่ ย่อแนวคิดวิธีทำให้สั้นลง 
step 1 : กำหนดก้อนใหญ่ก่อนโดยใช้ function "min" โดยการเลือก b,goal//5  คือจะเลือกค่าที่น้อยกว่า หมายความว่า จะใช้ชิ้นใหญ่ตามที่ต้องการ แต่ต้องไม่เกินจำนวนที่มีอยู่
step 2 : big * 5 คือ น้ำหนักรวมที่ได้จากของชิ้นใหญ่ที่เลือกใช้ re คือส่วนต่างที่เหลืออยู่
step 3 : ถ้า a > re ให้ปร้น a คือ ถ้าตัวสั้นมากกว่าส่วนต่างที่เหลือให้ปริ้นออกมาเลย แต่ถ้าน้อยกว่าให้ปริ้น -1
```
...
```text
เพื่อเช็คว่าถ้ามีชิ้นเล็กไม่พอ จะแสดงค่าออกมาได้ถูกไหม
```
...
```text
เพื่อดูว่าจะใช้ชิ้นเล็กได้ครบไหม
```
...
```text
เช็คว่าใช้ชิ้นเล็กได้ตามจำนวนที่ต้องการไหม
```
...
```text
ช่วยแนะนำว่ามีวิธีที่สั้นกว่านี้และง่ายกว่าอันแรกที่ทำอยู่
```

คุณยังทำอะไรด้วยตนเอง

```text
เขียนโค้ดเอง วางแผนกำหนดตัวต่างๆเอง 
```
