...
```text
โจทย์ให้คำนวณว่า ink จะไหลไปโดนบ้านในระยะเวลากี่วิ จะใช้เวลากี่วิถึงจะโดนบ้าน
input : รับตัวแปรเข้ามาแล้วเก็บไว้ใน list แปลงข้อมูลเป็น int อย่างแรกรับว่า ink แผ่ในรัศมีเท่าไร อย่างที่สอง จะมีบ้านกี่หลังที่จะให้คำนวณ ต่อมารับ for loop เพื่อรับจำนวนบ้าน n หลังตามที่ค่าอย่างที่สองกำหนด จากนั้นก็ input พิกัด x,y
output : แสดงค่าวินาทีที่บ้านแต่ละหลังจะโดนหมึก
constraint : พิจารณาการปัดเลขให้ถูกตามที่กำหนด
```
...
```text
รับ import math เข้ามาจากนั้นรับจำนวน input เข้ามา ตามที่โจทย์ต้องการก่อน คือ ระยะการแผ่และมีกี่บ้านที่ต้องการคิด หลังจากนั้นก็ใช้ลูปรับข้อมูล input อีกตัว ซึ่งจะวนรับค่าไปเรื่อยๆตามค่าที่รับจากบ้านที่กำหนด 
```
...
```text
คล้ายเดิม แต่เพิ่มค่าที่คำนวณเข้ามา เริ่มแรกคำนวณหาค่าของ r**2 ก่อน เพื่อจะได้รับรู้รัศมีที่วงกลมต้องแผ่ จากนั้นเข้าสูตร pi*r**2 เพื่อหาพท.วงกลม ต่อมาก็หาเวลาโดยการนำพท.วงกลมมาหารด้วยความการแผ่ต่อวิ หลังจากนั้นก็ print ค่า time โดยการใช้ math.ceil คู่ด้วยเพื่อจะได้ปัดทศนิยยมขึ้นให้
```
...
```text
ลองเช็คตามที่โจทย์กำหนด ว่าถ้าไม่เป็นจำนวนเต็มให้ปัดค่าขึ้น
```
...
```text
เพื่อเช็คว่าถ้าบ้านอยูที่ตำแหน่ง (0,0) พอดี จะตอบ 0 ไหม
```
...
```text
เพื่อเช็คว่าถ้ามีจำนวนบ้าน n หลัง โค้ดจะรันออกมาครบจำนวน n ไหม
```
...
```text
ช่วยแนะนำว่าต้องใช้สูตรอะไร คำนวณยังไง
```

คุณยังทำอะไรด้วยตนเอง

```text
เขียนโค้ด วางแผนการเขียนเอง 
```
