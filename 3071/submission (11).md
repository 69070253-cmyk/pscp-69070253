...
```text
input : รับค่าเลขจำนวนเต็มต่างๆมา 4 ตัว
output : มีกี่ตัวที่ใช้ d หารในช่วง [A,B] แล้วเหลือเศษ r
constraints : -
```
...
```text
step 1 : รับ input แต่ละตัวเข้ามาก่อน และกำหนดตัวแปรเพิ่ม 1 ตัวเพื่อเก็บค่าว่างเอาไว้
step 2 : จากนั้น loop ไปตั้งแต่ตัวที่ A ถึงตัวที่ B+1 +1 เพราะว่ามันจะนับถึงแค่ตัวก่อนท้ายเลยต้อง + ให้ถึงตัวท้ายพอดี
step 3 : เช็คถ้า i % d == r จะบวกเพิ่มทีละ 1 เรื่อยๆ ก็คือถ้าตัวใน loop i สามารถหารแล้วเหลือเศษเทากับ r พอดีให้ไป บวกเพิ่มทีละตัวใน x 
step 4 : แสดง output x ออกมา ว่ามีกี่ตัวที่หารแล้วเศษเท่ากับ r
```
...
```text
ใช้เหมือนวิธ๊แรก loop range A,B แล้วเช็คว่าสามารถหารแล้วได้เศษเท่ากับ r ไหม ถ้าใ่ช่ให้ +1 ทีละตัวไปที่ค่า x
```
...
```text
เช็คว่าบวกเพื่มทีละตัวไหม ถ้ามีค่าที่หารแล้วเท่ากับ r 
```
...
```text
เพื่อเช็คว่าถ้าไม่มีตัวที่หารแล้วเหลือเศษเท่ากับ r จะแสดงค่า 0 ออกมาไหม
