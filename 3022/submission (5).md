...
```text
โจทย์ให้รับค่าเป็นตัวเลขออกมาและรับค่าเป็นตัวที่กำหนดอันที่ 2 และแสดงผลลัพท์ออกมาเป็นที่กำหนดอันที่ 3
input : รับค่าอุณหภูมิเป็น float และรับค่าตัวที่ 2 กับ 3 เป็น str
output : แสดงผลออกมาหลังจากเปลี่ยนค่าเรียบร้อยแล้ว 
Contraints : input ตัวเลขเป็น float และต้องไม่ลืมทำให้เป็นทศนิยม  ตำแหน้ง
```
...
```text
step 1  : รับค่าตัวเลขเป็น float  และอีก 2 ตัวเป็น str
step 2 : สร้างสมการทำให้้ค่าต่างๆที่กำหนดมาทุกตัวแปลงมาเป็นค่า C 
```
...
```text
วิธีนี้หลังจากเพื่อนได้ช่วยแนะนำวิธีทำแล้ว
กำหนด ค่า ว่าง ออกมา 2 ตัว เพื่อที่จะทำให้ค่าที่เขียนขึ้นมาใหม่แสดงผลออกมา
หลังจากที่แปลงค่าต่างๆเป็นค่า  C แล้ว
นำค่าที่แปลงเป็น C มาเขียนเป็นสมการที่แสดงผลออกมาเป็น C K F R ตามที่โจทย์์กำหนดโดยใช้สมการที่โจทย์ให้มา
โดยกำหนดค่าว่างออกมาอีกตัวนึงเพื่อเป็ยตัวแทนของ C K F R 
```
...
```text
เพื่อเช็คว่าโค้ดสามารถทำงานได้ปกติ
```
...
```text
เพื่อเช็คว่าโค้ดสามารถรองรับค่าติดลบและคำนวณออกมาได้ถูฏต้อง
```
...
```text
จะเป็นตัวที่อาจจะเกิด error ได้ง่ายสุด เพื่อเช็คว่าสมการและการลบค่าที่โจทย์มีให้นั้นแสดงผลออกมาได้
```
...
```text
ช่วยแนะนำว่าหลังจากที่แปลงทุกตัวเป็น C ยังต้องนำมาทำต่ออีก 
```

คุณยังทำอะไรด้วยตนเอง

```text
เขียนสมการแปลงค่าทุกตัวเป็น C เอง เขียนวิธีการดำเนินการต่างๆเอง เขียนโค้ดเอง
```
