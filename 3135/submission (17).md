...
```text
input : ต้องการรับจำนวนคนทั้งหมด ต้องการรับค่าว่าต้องส่งของข้ามไปกี่คน และต้องการบอกว่าโจรอยู่ตำแหน่งไหน
output : มีคนที่ได้ตรวจสอบของกี่คน
constraints : ไม่นับคนตรวจสอบซ้ำและหยุดทันทีเมื่อถึงมือโจร
```
...
```text
step 1 : กำหนดค่า input ทั้ง 3 ตัว และกำหนดตัวแปรเพิ่มตัวแรกให้ = 1 แทนค่าเริ่มต้นคนที่ 1 และกำหนดอีกตัวให้ = 1 เพราะว่าให้นับคนแรกที่ตรวจสอบด้วย ถ้าเป็น 0 จะไม่นับ
step 2 : ใช้ while loop วนไปเรื่อยๆ จากนั้นกำหนดสมการให้ส่งของขวัญตามตำแหน่งที่บอก(ให้คนถัดไปเรื่อยๆ)
step 3 : ถ้าวนชองวนกลับมาที่คนที่ 1 ให้หยุดทันทีและ ให้บวก เพิ่มเข้าไปในตัวแปร count
step 4 : กำหนดอีกตัวถ้าของอยู่ที่โจรให้หยุดทันที จากนั้นแสดงค่า count ออกมา
```
...
```text
ทำเหมือนโค้ดแรกแต่เพิ่ม if เข้ามาในกรณีทีถ้า โจรคือคนแรกที่ได้ตรวจสอบของ ให้แสดงค่า count ออกมาเลย
และก็เพิ่ม else ไว้บน while loop
```
...
```text
เช็คว่าถ้าของไปตกอยู่โจร จะหยุดทำงานไหม
```
...
```text
เช็คว่าถ้าของไม่ได้ตกไปที่โจรก่อนและวนกลับมาที่เลข 1 จะหยุดทำงานโค้ดไหม และเช็คว่าถ้าวนกลับมาซ่ำตัวเดิมจะไม่เพิ่มค่า
```
...
```text
เช็ึคถ้าโจรอยู่ที่ตำแหน่งเริ่มแล้วโค้ดจะหยุดเลยไหม แล้วแสดงค่าออกมาได้ถูกต้องไหม
```
