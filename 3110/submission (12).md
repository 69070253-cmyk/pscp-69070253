...
```text
input : รับค่าต้นทางและปลายทางในบรรทัดเดียวกัน และรับค่าน้ำหนักของสินค้าเป็น float
output : แสดงค่าว่าจากต้นทางที่นึงไปถึงอีกที่นึงจะเสียเงินเท่าไร และถ้าไม่มีสถานที่ ที่กำหนดในตารางให้แสดง Error
constraints : ถ้าต้นทางกับปลายทางไม่ตรงกับในตารางเลยต้องแสดงอีกค่านึง
```
...
```text
step 1 : รับค่ากำหนด split ขึ้นมา 1 ตัว แล้วหลังจากนั้นใช้ 2 ตัวแปรในการรับค่าต้นทาง ปลายทางในบรรทัดเดียว และก็รับน้ำหนักสินค้า
step 2 : กำหนดเงื่อนไขทั้งหมดตามตาราง อย่างเช่น if first == "BKK" and last == "CNX" ก็คือรับค่าต้นทางเป็น BKK และปลายทางเป็น CNX แล้วหลังจากนั้นให้อแสดงผลออกมาโดยการนำ ค่าธรรมเนียม + (น้ำหนัก * ราคาต่อน้ำหนัก) และก็ทำอย่างงี้ไปเรื่อยๆ จนครบ 
step 3 : เพิ่ม f string เข้าไปในแต่ละตัวเพื่อให้เป็นทศนิยม 2 ตน. แล้วก็กำหนด else ให้แสดงค่า Error
```
...
```text
ทำแบบขั้นตอนแรก รับเงื่อนไขตามตารางจนครบ แสดงผลเป็นทศนิยม 2 ตน. ถ้าไ่ม่มีต้นทาง ปลายทางตามตารางให้แสดง error
```
...
```text
เพื่อเช็คว่าเงื่อนไขที่เขียนนั้นถูกต้องและแสดงผลออกมาเป็นทศนิยม 2 ตน.
```
...
```text
เพื่อเช็คว่าถ้าไม่ตรงตามตารางเลย จะแสดงค่าออกมาได้คามที่กำหนดไหม
```
...
```text
เพื่อเช็คว่าถ้าเป็นจำนวนทศนิยมจะแสดงค่าออกมาได้ไหม และเพื่อเช็คว่าจะแปลงเป็นตัวพิมพ์ใหญ๋ไหม
```
