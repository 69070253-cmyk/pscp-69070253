...
```text
โจทย์ให้หาค่าที่หาร 10 ลงตัวและลดไปทีละ 10 เรื่อยๆ
input : รับจำนวนเต็มมา 1 จำนวน
output : แสดงค่าที่หาร 10 ลงตัว และลดลงทีละลงไปเรื่อยๆจนถึงค่า 0 
constraint : คิดค่าที่หาร 10 ไม่ลงตัวพอดีว่าต้องแก้ยังไง
```
...
```text
รับ input เป็นจำนวนเต็มเข้ามาก่อน จากนั้นสร้าง for loop เพื่อมาเช็คว่าจะหาร 10 ลงตัวไหมจากการนำไป //10 ในช่วง start แลัวจากนั้นก็ -1 ทั้ง 2 step นั่นก็คือ stop, step เพราะว่า ที่ stop เป็น -1 เพราะจะได้หยุดที่ 0 พอดี ส่วน step เพราะจะได้เริ่มไล่เลขจากหลังมาหน้า แล้วหลังจากนั้นนำตัวเลขที่นับถอยหลังได้มา *10 เพื่อให้เลขกลับมากลายเป็นหลัก 10  แล้วก็ + ช่องว่างเข้าไป จากนั้นก็ print ออกมา
```
...
```text
เหมือนแผนแรกเลย เพราะ วิธ๊แรกที่ใช้นั้นถูดต้องแล้ว
```
...
```text
เพื่อเช็คดูว่าถ้าไม่ถึง 10 ค่าจะออกมาเป็น 0 ไหม
```
...
```text
เพื่อดูว่าถ้าหาร 10 ลงตัว จะแสดงผลลัพท์ออกมาตากที่ต้องการไหม
```
...
```text
เพื่อเช็คว่าถ้าตัวที่ไม่ใช่ 10 หารลงตัวตรงๆ จะแสดงค่าที่หาร 10 ลงตัวออกมาไหม 
```
