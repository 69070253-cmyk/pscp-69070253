...
```text
โจทย์ให้หาค่าถ้าคะแนนห่างกันเกิน 2 ให้แสดงค่านึงออกมา แต่ถ้าน้อยกว่า 2 ให้แสดงอีกค่านึงออกมา
input :  รับค่าคะแนนรวมของนักเรียนทั้ง 3 คน และรับค่าคนที่มีคะแนนสูงสุง
output : โปรแกรมจะแสดง Surprising ถ้ามีคะแนนมากกว่า 2 แต่ถ้าคะแนนน้อยกว่า 2 จะแสดง Not surprising
Constraints : ต้องเช็คเงื่อนไขว่่าถ้าน้อยกว่า 0 ให้ค่าเปลี่ยนเป็น 0
```
...
```text
step 1 : รับค่า ผลรวมของ 3 คนและรับค่าคะแนนสูงสุดของ 3 คน
step 2 : หาค่าว่าผลรวมของทั้ง 3 คนนั้นแต่ละคนมีกีคะแนนกันบ้าง โดยตั้งสมการคณิต
step 3 : ตั้งว่า ผลรวม 3 คน - คะแนนสูงสุดใน 3 คน = คะแนนของ 2 คนรวมกัน 
...
```text
คล้ายๆเดิมแต่่เพิ่มวิธีการขึ้น
step 3 จากอันที่แล้ว เปลี่ยนเป็น ผลรวม 3 คน - 2 * คะแนนสูงสุด = ค่าที่น้อยที่สุดที่เป็นไปได้
และก็กำหนดว่าถ้าค่าที่ได้นั้นออกมาเป็นติดลบ ให้เปลี่ยนเป็น 0
หลังจากนั้นนำ ค่ามากสุด - ค่าน้อยสุดจะได้ผลต่างออกมา
และก็กำหนดเงื่อนไขว่าถ้า ผลต่าง > 2 จะแสดงผลนี้ออกมา ถ้า <= จะแสดงอีกผลลัพท์ออกมา
```
...
```text
เช็คว่าเงื่อนไขที่ตั้งมาแสดงผลได้ถูกไหม
```
...
```text
เช็คว่าถ้ามีค่ามากสุดเท่ากันจากสมการที่ตั้ง ค่าที่เหลืออีกค่าจะกลายเป็น 0 และทำให้แสดงผลออกมาได้ถูก
```
...
```text
เช็คว่าถ้ามีแค่ค่าเดียวแล้วจะแสดงผลลัพท์ออกมาถูกต้องไหม
```
...
```text
ช่วยบอกว่าถ้าค่าที่ได้ออกมาจากสมการเป็นค่าติดลบให้เปลี่ยนเป็น 0
```

คุณยังทำอะไรด้วยตนเอง

```text
ตั้งสมการหลักด้วยตัวเอง เขียนวิธ๊การทำต่างๆเอง เพื่อนแค่ช่วยแนะนำว่าถ้าค่านี้ผลออกมาเป็นลบจะแก้ยังไง
```
