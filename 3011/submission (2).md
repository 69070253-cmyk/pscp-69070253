...
```text
โจทย์ต้องการให้นำสีที่กำหนดให้ต่างๆมารวมกันกลายเป็นอีกสีนึง
input : รับค่า str เข้ามา
output : แสดงค่าสีที่รวมกันออกมาให้ถูกต้อง และถ้ามีข้อความที่ไม่ตรงกับสีที่กำหนดจะปริ้น error ออกมา
constraints : ค่าที่แสดงออกมาถูกหมดตาม testcase แต่ไม่ได้เช็คว่าถ้าเป็นสีที่เหมือนกันและรับค่าสีสลับกันจะทำให้มันขั้น error เลยต้องเพิ่มเงื่อนไขเพิ่ม
```
...
```text
1.รับค่า input เข้ามา
2.ใช้วิธีการนำสีมาบวกกัน
3.รู้ว่ามัน print แต่ค่าเดิมๆออกมา
4.ผิด
```
...
```text
คล้ายแผนแรก 
1.แต่เปลี่ยนจากนำมาบวกกันเป็นกำหนดค่าให้ตรงตัว แล้วก็นำตัวแปรมาสลับกันเวลาเขียนสลับที่กันจะได้ไม่ขั้น error 
2.เพิ่มขั้นตอนสีเดียวกันรันออกมาเป็นตัวเดียวกัน
3.เขียนขั้นตอนที่ถ้ามีสีไม่ตรงกับที่กำหนดจะขึ้น error
```
...
```text
เป็น testcase ที่ลืมไปในครั้งแรกเลยทำให้ไม่ผ่าน
```
...
```text
เขียนสลับกับที่ sample testcase กำหนดไว้ ค่าตอนแรกที่ทำขึ้น error
...
```text
เช็คว่ามันจะ error ตามที่เขียนใน code จริงไหม
```
...
```text
ช่วยบอกค่าที่ขาดใน testcase ให้
```

คุณยังทำอะไรด้วยตนเอง

```text
เขียน code เอง ทดสอบเอง
```
