...
```text
input : ต้องการรับจำนวนร้านค้า และเวลาที่ต้องการเช็ค และเวลาเปิด-ปิดของแต่ละร้านเป็นนาที
output : ต้องการว่ามีร้านที่เปิดในช่วงเวลาที่ต้องการอยู่กี่ร้าน
constraints : ต้องแสดงค่าออกมาโดยมีช่องว่างกั้นไว้และไม่เอาเวลาที่ปิดพอดี
```
...
```text
step 1 : ฉันต้องการใช้ split ในการรับค่าของ จำนวนร้านค้าและเวลาที่ต้องเช็คว่ามีกี่เวลาที่อยากเช็ค
step 2 : ฉันใช้ for loop ในการวนรับค่า split เพื่อให้มีจำนวนตามที่โจทย์ต้องการ และฉันสร้าง list เอาไว้เก็บช่วงเวลา เปิด-ปิด ของร้าน
```
...
```text
ต่อจากแผนแรกของฉัน แต่หลังจากขั้นตอนนั้นฉันใช้ ai ในการช่วยแสดงแนวทางในการใช้ for loop และ split
step 3 : ฉันใช้ loop ในการรับค่าช่วงเวลาที่ต้องการเช็ค จากนั้นสร้างตัวแปรมาเก็บ split ไว้ใน loop ของ i และฉันก็เก็บข้อมูลที่ loop มาได้นั้นไว้ใน list 
step 4 : สร้างอีก loop เพื่อเช็คค่าเวลาเปิด-ปิด โดยการ loop ตัวแปรที่เก็บช่วงเวลาเปิด-ปิดร้านไว้ จากนั้นใช้ if ในการกำหนดว่าถ้าช่วงเวลา ถ้าเวลาที่ต้องการหาอยู่ในช่วงเวลาเปิด-ปิด ให้ +1 เข้าไปในตัวแปร 0 และก็นำไปเก็บไว้ใน list 
step 5 : แปลงค่าที่ไได้ใน list มาเป็น str หลังจากนั้นแสดงผลออกมาโดยใช้ .join ในการสร้างช่องว่างขึ้นมาแทนเข้าไปในทีละตัว
```
...
```text
เพื่อเช็คว่าถ้าอยู่ในช่วงเวลาที่ร้านปิดพอดีจะแสดงผล +1 ออกมาไหม
```
...
```text
เพื่อเช็คว่าสามารถเช็คช่วงเวลาต่างๆได้จริงๆ และแสดงค่าออกมามีช่องว่างกั้นไว้
```
...
```text
เพื่อดูว่าถ้ามีทั้งช่วงเวลาที่ร้านเปิด ช่วงเวลาที่อยู่ในระหว่างเปิด-ปิด และช่วงเวลาที่ร้านปิด จะแสดงออกมาได้ครบไหม
```
