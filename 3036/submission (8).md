...
```text
โจทย์ต้องการทำลายกำแพงหาสมบัติ โดยใช้ทางที่สั้นที่สุด
input : รับค่าจำนวนเต็ม
output : กำแพงที่ต้องพังเพื่อไปให้ถึงห้องสมบัติโดยใช้ทางที่สั้นที่สุด ละถ้าอยู่ห้องสมบัติอยู่แล้วให้ตอบ 0
constraint : พิจารณาว่าถ้าอยู่ห้องทีมีสมบัติอยู่แล่วตอบ 0 ไหม
```
...
```text
เขียนวิธีคำนวฯขึ้นมาก่อนว่าจะหาได้ยังไงว่าเราอยู่ชั้นไหน โดยการนำค่าห้องที่เราอยู่ไปเข้า sqrt แล้วปัดเศษขึ้น เพราะว่าในแต่ละแถวตัวสุดท้ายเป็นตัวที่ แถว**2 ทุกตัว เลยทำให้ถ้าปัดเศษขึ้นแล้วจะสามารถเช็คได้ว่าอยู่ชั้นไหน
```
...
```text
ต่อจากตอนแรกเพื่อนช่วยบอกแนวคิด ถ้าอยู่ในชั้นที่เป็นเลขคู่และเป็นเลขคู่ และชั้นเลขคี่และเป็นเลขคี่ โดยการใช้สูตร (ชั้น - 1) * 2 จากนั้นก็ทำคล้ายเดิมแค่กลับกัน ถ้าอยู่ในชั้นเลขคี่และเป็นเลขคู่ และชั้นเลขคู่ที่เป็นเลขคี่ ก็จะเข้าสูตรคล้ายเดิมแต่เพิ่ม -1 มาด้วย ก็จะทำให้รู้ว่าในส่วนที่ 2 นี้จะสามารถทำลายกำแพงไปได้เร็วกว่า 1 ชั้น
```
...
```text
เพื่อเช็คว่าถ้าอยู่ในห้องที่มีสมบัติแล้ว คำตอบจะเป็็น 0 ไหม
```
...
```text
เพื่อดูว่าถ้าอยู๋ชั้นคู่เลขคู่ จะทำถูกตามที่เขียนโค้ดไหม
```
...
```text
เพื่อเช็คว่าถ้าอยู่ชั้นคู่เลขคี่ จะเข้าไปในโค้ดได้ถูกต้องไหมและจะได้คำตอบตรงกับที่คิดไหม
```
...
```text
ช่วยไกด์ไลน์จะหาวิธ๊ที่ทำลายกำแพงได้เร็วที่สุดได้ยังไง
```

คุณยังทำอะไรด้วยตนเอง

```text
เขียนโค้ดด้วยตัวเอง ทดสอบ testcase ที่เป็นไปได้เอง
```
